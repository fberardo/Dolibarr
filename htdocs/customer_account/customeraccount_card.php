...
/**
 *   	\file       customer_account/customeraccount_card.php
...
if (empty($reshook))
{
	if ($cancel) 
	{
		if ($action != 'addlink')
		{
			$urltogo=$backtopage?$backtopage:dol_buildpath('/customer_account/list.php',1);
	...
	if ($action == 'confirm_delete')
	{
		...
		if ($result > 0)
		{
			...
			header("Location: ".dol_buildpath('/customer_account/list.php',1));
			exit;
		}
		else
		{
...
print '<tr><td class="fieldrequired">'.$langs->trans("Fieldactive").'</td><td>'.$object->active.'</td></tr>';
	...
	if (empty($reshook))
	{
		if ($user->rights->customer_account->write)
		{
			print '<div class="inline-block divButAction"><a class="butAction" href="'.$_SERVER["PHP_SELF"].'?id='.$object->id.'&amp;action=edit">'.$langs->trans("Modify").'</a></div>'."\n";
		}

		if ($user->rights->customer_account->delete)
		{
