...
/**
 *   	\file       customer_account/customeraccount_list.php
 *		\ingroup    customer_account
...
dol_include_once('/customer_account/class/customeraccount.class.php');

// Load traductions files requiredby by page
$langs->load("customeraccount");
$langs->load("other");
...
$contextpage=GETPOST('contextpage','aZ')?GETPOST('contextpage','aZ'):'customer_accountlist';

// Initialize technical object to manage hooks. Note that conf->hooks_modules contains array
$hookmanager->initHooks(array('customer_accountlist'));
$extrafields = new ExtraFields($db);

// fetch optionals attributes and labels
$extralabels = $extrafields->fetch_name_optionals_label('customer_account');
...
$search_active='';
    ...
    $permtoread = $user->rights->customeraccount->read;
    $permtodelete = $user->rights->customeraccount->delete;
    $uploaddir = $conf->customeraccount->dir_output;
...
if (is_array($extrafields->attribute_label) && count($extrafields->attribute_label)) $sql.= " LEFT JOIN ".MAIN_DB_PREFIX."customer_account_extrafields as ef on (t.rowid = ef.fk_object)";
$sql.= " WHERE 1 = 1";
...
if ($num == 1 && ! empty($conf->global->MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE) && $search_all)
{
    ...
    $id = $obj->rowid;
    header("Location: ".DOL_URL_ROOT.'/customeraccount/card.php?id='.$id);
    exit;
}
...
if ($user->rights->customer_account->supprimer) $arrayofmassactions['delete']=$langs->trans("Delete");
...
if ($massaction == 'builddoc' || $action == 'remove_file' || $show_files)
{
    ...
    print $formfile->showdocuments('massfilesarea_customer_account','',$filedir,$urlsource,0,$delallowed,'',1,1,0,48,1,$param,$title,'');
}
else
{
