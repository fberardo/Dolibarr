...
/**
 *   	\file       supplier_account_movement/supplieraccountmovement_list.php
...
require_once DOL_DOCUMENT_ROOT.'/supplier_account/class/supplieraccountmovement.class.php';
...
//$contextpage=GETPOST('contextpage','aZ')?GETPOST('contextpage','aZ'):'supplier_account_movementlist';
$contextpage='supplier_account_movementlist'.(empty($object->ref)?'':'-'.$object->id);


// Initialize technical object to manage hooks. Note that conf->hooks_modules contains array
$hookmanager->initHooks(array('supplier_account_movementlist'));
$extrafields = new ExtraFields($db);

// fetch optionals attributes and labels
$extralabels = $extrafields->fetch_name_optionals_label('supplier_account_movement');
...
$arrayfields=array(
    ...
    't.dateo'=>array('label'=>$langs->trans("SupplierAccountFieldDateOperationShort"), 'checked'=>1),
    't.amount'=>array('label'=>$langs->trans("SupplierAccountFieldamount"), 'checked'=>1),
    't.label'=>array('label'=>$langs->trans("SupplierAccountFieldlabel"), 'checked'=>1),
    't.datec'=>array('label'=>$langs->trans("SupplierAccountDateCreationShort"), 'checked'=>0, 'position'=>500),
...
$permtoread = $user->rights->supplieraccount->supplieraccountmovement->read;
...
if (empty($reshook))
{
    ...
    $uploaddir = $conf->supplieraccountmovement->dir_output;
...
//$help_url="EN:Module_Suppliers_Orders|FR:Module_Commandes_Clients|ES:Módulo_Pedidos_de_clientes";
$help_url='';
$title = $langs->trans('SupplierAccountListTitle');
...
$sql.= " FROM ".MAIN_DB_PREFIX."supplier_account_movement as t";

$sql.= " INNER JOIN ".MAIN_DB_PREFIX."supplier_account as a ON (t.fk_supplier_account = a.rowid)";
...
if (is_array($extrafields->attribute_label) && count($extrafields->attribute_label)) $sql.= " LEFT JOIN ".MAIN_DB_PREFIX."supplier_account_movement_extrafields as ef on (t.rowid = ef.fk_object)";
//$sql.= " WHERE 1 = 1";
...
if ($num == 1 && ! empty($conf->global->MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE) && $search_all && $permtoread)
{
    ...
    header("Location: ".DOL_URL_ROOT.'/supplier_account/supplieraccountmovement_card.php?id='.$obj->rowid.'&socid='.$socid.$hiddenobjvaluesedit);
...
if ($user->rights->supplier_account_movement->supprimer) $arrayofmassactions['delete']=$langs->trans("Delete");
...
$sql2 = "SELECT";
$sql2.= " rowid";
$sql2.= " FROM ".MAIN_DB_PREFIX."supplier_account";
...
$hiddenobjvaluesnew='&amp;entity=1'.
        '&amp;fk_supplier_account='.$obj2->rowid.
        '&amp;active=1';

if ($permtowrite)
{
    $newbutton.='<a class="butAction" href="supplieraccountmovement_card.php?action=create&socid='.$socid.$hiddenobjvaluesnew.'">'.$langs->trans("SupplierAccountNewSupplierAccountMovement").'</a>';
}
...
$moreforfilter .= $langs->trans('SupplierAccountFieldDateOperationShort').' : ';
            ...
            if (!empty($arrayfields[$key]['checked'])) {
                ...
                if ($key2 == 'rowid')
                {
                    ...
                    if ($permtoread)
                    {
                        ...
                        print "<a href=\"supplieraccountmovement_card.php?id=".$obj->rowid.'&socid='.$socid.$hiddenobjvaluesview.'">'.img_object($langs->trans("SupplierAccountShowSupplierAccountMovement").': '.$obj->rowid, 'account', 'class="classfortooltip"').' '.$obj->rowid."</a> &nbsp; ";
                        print '</td>';
                    }
                    else
                    {
...
if (isset($totalarray['totalhtfield']))
{
    ...
    while ($i < $totalarray['nbfield'])
    {
        ...
        if ($i == 1)
        {
            if ($num < $limit && empty($offset)) print '<td align="left"><span style="font-weight:bold">'.$langs->trans("SupplierAccountTotal").'</span></td>';
            else print '<td align="left">'.$langs->trans("SupplierAccountTotalforthispage").'</td>';
        }
...
print '<th class="liste_titre">'.$langs->trans("SupplierAccountSaldoCuentaCorriente").'</th>';
print '<th class="liste_titre">'.$langs->trans("SupplierAccountRestaCobrarFacturas").'</th>';
print '<th class="liste_titre">'.$langs->trans("SupplierAccountSaldoCliente").'</th>';
print '</tr>';
// Row values
print '<tr class="liste_titre">';
print '<td>'.$langs->trans("SupplierAccountBalanceCuentadelProveedor").'</td>';
...
/*if ($massaction == 'builddoc' || $action == 'remove_file' || $show_files)
{
    ...
    print $formfile->showdocuments('massfilesarea_supplier_account_movement','',$filedir,$urlsource,0,$delallowed,'',1,1,0,48,1,$param,$title,'');
}
else
{
