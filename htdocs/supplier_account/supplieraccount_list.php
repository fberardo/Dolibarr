...
/**
 *   	\file       supplier_account/supplieraccount_list.php
 *		\ingroup    supplier_account
 *		\brief      This file is an example of a php page
 *					Initialy built by build_class_from_table on 2017-08-29 19:37
 */
...
dol_include_once('/supplier_account/class/supplieraccount.class.php');

// Load traductions files requiredby by page
$langs->load("supplieraccount");
$langs->load("other");
...
$search_fk_societe=GETPOST('search_fk_societe','int');
...
$contextpage=GETPOST('contextpage','aZ')?GETPOST('contextpage','aZ'):'supplier_accountlist';

// Initialize technical object to manage hooks. Note that conf->hooks_modules contains array
$hookmanager->initHooks(array('supplier_accountlist'));
$extrafields = new ExtraFields($db);

// fetch optionals attributes and labels
$extralabels = $extrafields->fetch_name_optionals_label('supplier_account');
...
't.fk_societe'=>array('label'=>$langs->trans("Fieldfk_societe"), 'checked'=>1),
...
$search_active='';
    ...
    $permtoread = $user->rights->supplieraccount->read;
    $permtodelete = $user->rights->supplieraccount->delete;
    $uploaddir = $conf->supplieraccount->dir_output;
...
$sql.= " FROM ".MAIN_DB_PREFIX."supplier_account as t";
if (is_array($extrafields->attribute_label) && count($extrafields->attribute_label)) $sql.= " LEFT JOIN ".MAIN_DB_PREFIX."supplier_account_extrafields as ef on (t.rowid = ef.fk_object)";
$sql.= " WHERE 1 = 1";
//$sql.= " WHERE u.entity IN (".getEntity('mytable',1).")";

if ($search_entity) $sql.= natural_search("entity",$search_entity);
if ($search_label) $sql.= natural_search("label",$search_label);
if ($search_fk_societe) $sql.= natural_search("fk_societe",$search_fk_societe);
...
if ($num == 1 && ! empty($conf->global->MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE) && $search_all)
{
    ...
    $id = $obj->rowid;
    header("Location: ".DOL_URL_ROOT.'/supplieraccount/card.php?id='.$id);
    exit;
}
...
if ($user->rights->supplier_account->supprimer) $arrayofmassactions['delete']=$langs->trans("Delete");
...
if (! empty($arrayfields['t.fk_societe']['checked'])) print_liste_field_titre($arrayfields['t.fk_societe']['label'],$_SERVER['PHP_SELF'],'t.fk_societe','',$params,'',$sortfield,$sortorder);
...
if (! empty($arrayfields['t.fk_societe']['checked'])) print '<td class="liste_titre"><input type="text" class="flat" name="search_fk_societe" value="'.$search_fk_societe.'" size="10"></td>';
...
/*if ($massaction == 'builddoc' || $action == 'remove_file' || $show_files)
{
    ...
    print $formfile->showdocuments('massfilesarea_supplier_account','',$filedir,$urlsource,0,$delallowed,'',1,1,0,48,1,$param,$title,'');
}
else
{
